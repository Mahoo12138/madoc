...
type Props = {
  ...
  versionsRequest?: number;
  versionsRequestItemId?: string;
};
...
export function MarkdownEditor({
  ...
  onOutlineChange,
  versionsRequest = 0,
  versionsRequestItemId,
}: Props) {
  const initial = useMarkdown(item.id);
  const initialRole = useRef(role);
  useEffect(() => {
    if (role === "viewer" && initialRole.current !== "viewer")
      realtimeRef.current?.rejectItem(item.id);
  }, [role, item.id]);
  ...
  useEffect(() => {
    if (versionsRequest > 0 && versionsRequestItemId === item.id)
      setVersionsOpened(true);
  }, [item.id, versionsRequest, versionsRequestItemId]);
  ...
  const leaveGuard = useRef<{
    unsafe: () => boolean;
    settle: () => Promise<void>;
  }>({ unsafe: () => false, settle: async () => {} });
  useBlocker({
    shouldBlockFn: async () => {
      await leaveGuard.current.settle();
      return (
        leaveGuard.current.unsafe() &&
        !window.confirm(
          "修改尚未保存到此设备。离开会丢失这些修改，仍要离开吗？",
        )
      );
    },
  ...
  const [inlinePreview, setInlinePreview] = useState<InlineMathPreview | null>(
    null,
  );
  ...
  const toggleFocusMode = () =>
    setPreferences({ focusMode: !preferencesRef.current.focusMode });
  const toggleTypewriterMode = () =>
    setPreferences({ typewriterMode: !preferencesRef.current.typewriterMode });
  ...
  useEffect(() => {
    ...
    const stop = startMarkdownSession({
      ...
      onExportReady: (source) => {
        ...
        unregisterCopy?.();
        if (source)
          unregisterCopy = registerContentSave(item.id, async (signal) => {
            await exportConfirmedMarkdown(item.id, source, signal);
          });
      },
      onFindReady: (controller) => {
      ...
      onFailure: (message, download, retry) => {
        if (active) setFailure({ message, download, retry });
      },
      onRecovered: () => {
    ...
    item.id,
    item.workspaceId,
    initial.data?.cacheSeq,
    initial.data?.markdown,
    user.id,
    onOutlineChange,
  ]);
  ...
  const editorClassName = [
    styles.editor,
    !readingMode && focusMode ? styles.focusMode : "",
    !readingMode && typewriterMode ? styles.typewriterMode : "",
  ]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    ...
    >
      {readingMode ? (
        <h1 className={styles.title} data-testid="markdown-title">
          {title || "无标题文档"}
        </h1>
      ) : (
        <input
      ...
      {exporter.error && (
        <Alert color="orange" title="导出尚未完成" role="alert">
          {exporter.error}
          <Button variant="light" onClick={() => void exporter.retry()}>
            重试导出
          </Button>
          <Button variant="light" onClick={exporter.local}>
            下载本地副本
          </Button>
        </Alert>
      )}
      {failure && (
        <Alert color="red" title="保存已停止" role="alert">
          {failure.message}
          <Button variant="light" onClick={failure.download}>
            下载本地副本
          </Button>
          {failure.retry && (
            <Button variant="light" onClick={failure.retry}>
              重试本地保存
            </Button>
          )}
        </Alert>
      ...
      {versionsOpened && (
        <VersionHistoryDialog
          ...
          assetIds={() => [
            ...new Set(
              exporter
                .assetReferences(exporter.inspect().markdown)
                .map((source) =>
                  localMadocAssetID(source, window.location.origin),
                )
