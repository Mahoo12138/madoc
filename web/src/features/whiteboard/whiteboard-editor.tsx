...
export function WhiteboardEditor({ item, role, user }: { item: Item; role: Role; user: User }) {
  ...
  const changeVersion = useRef(0); const sentVersions = useRef<number[]>([]);
  useEffect(() => {
    ...
    const unsubscribe = realtime.subscribe((message) => {
      ...
      if (message.type === 'whiteboard.scene.ack') { const acknowledged = sentVersions.current.shift(); const pending = acknowledged !== changeVersion.current; setPendingChanges(`whiteboard:${item.id}`, pending); setRevision((message.payload as { revision: number }).revision); setStatus(pending ? 'Saving' : 'Saved'); }
    ...
    return () => { changeVersion.current = 0; sentVersions.current = []; setPendingChanges(`whiteboard:${item.id}`, false); window.clearTimeout(timerRef.current); realtime.send('room.leave', item.id); unsubscribe(); realtime.close(); };
  ...
  const onChange = (elements: readonly unknown[], appState: Record<string, unknown>, files: Record<string, unknown>) => {
    if (remoteRef.current || role === 'viewer') return; const version = ++changeVersion.current; setPendingChanges(`whiteboard:${item.id}`, true); setStatus(realtimeRef.current?.state === 'online' ? 'Saving' : 'Offline'); window.clearTimeout(timerRef.current);
    timerRef.current = window.setTimeout(() => { sentVersions.current.push(version); realtimeRef.current?.send('whiteboard.scene.update', item.id, { baseRevision: revision, scene: { elements, appState: durableAppState(appState), files } }); }, 350);
  };
