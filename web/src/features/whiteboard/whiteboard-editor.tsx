...
export function WhiteboardEditor({ item, role, user }: { item: Item; role: Role; user: User }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = realtime.subscribe((message) => {
      ...
      if (message.type === 'whiteboard.init' || message.type === 'whiteboard.scene.remote') {
        ...
        if (api) {
          ...
          api.updateScene({ elements, appState: payload.scene.appState as never });
        }
        apiRef.current?.addFiles(Object.values(payload.scene.files ?? {}) as never[]);
        window.setTimeout(() => { remoteRef.current = false; }, 0);
        setStatus(confirmedVersion.current < changeVersion.current ? 'Saving' : 'Saved');
      }
      if (message.type === 'whiteboard.scene.ack') {
        const acknowledged = sentVersions.current.shift();
        if (acknowledged === undefined) return;
        confirmedVersion.current = Math.max(confirmedVersion.current, acknowledged);
        const pending = confirmedVersion.current < changeVersion.current;
        setPendingChanges(`whiteboard:${item.id}`, pending);
        setRevision((message.payload as { revision: number }).revision);
    ...
    return () => { lastScene.current = undefined; confirmedVersion.current = 0; changeVersion.current = 0; sentVersions.current = []; setPendingChanges(`whiteboard:${item.id}`, false); window.clearTimeout(timerRef.current); realtime.send('room.leave', item.id); unsubscribe(); realtime.close(); };
  ...
  const onChange = (elements: readonly unknown[], appState: Record<string, unknown>, files: Record<string, unknown>) => {
    ...
    if (remoteRef.current || role === 'viewer') return;
    const version = ++changeVersion.current;
    setPendingChanges(`whiteboard:${item.id}`, true);
    setStatus(realtimeRef.current?.state === 'online' ? 'Saving' : 'Offline');
    window.clearTimeout(timerRef.current);
    timerRef.current = window.setTimeout(() => {
      sentVersions.current.push(version);
      realtimeRef.current?.send('whiteboard.scene.update', item.id, { baseRevision: revision, scene });
    }, 350);
  };
