...
import { downloadPortablePackage, exportFolderPackage } from './folder-portable-export';
...
export function FolderExportDialog({ folder, items, onClose }: { folder: Item; items: Item[]; onClose: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      notifications.show({ message: `文件夹导出完成：${result.itemCount} 个内容、${result.attachmentCount} 个附件；${result.externalImageCount} 个外部图片仍使用原地址。` });
      onClose();
    } catch (failure) {
  ...
  return (
    <Modal opened title={`导出文件夹：${folder.title}`} onClose={busy ? () => {} : onClose} closeOnClickOutside={!busy} closeOnEscape={!busy} withCloseButton={!busy}>
      <Stack>
        <Text size="sm">将按每篇内容分别捕获服务端已保存版本，并记录各自水位和时间。未确认修改不会包含；这不是文件夹同一时刻的快照。</Text>
