...
import { downloadPortablePackageSet, exportFolderPackage } from './folder-portable-export';
...
export function FolderExportDialog({ folder, items, onClose }: { folder: Item; items: Item[]; onClose: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      notifications.show({ message: `文件夹导出完成：${result.packages.length} 个 ZIP 分包、${result.itemCount} 个内容、${result.attachmentCount} 个附件；${result.externalImageCount} 个外部图片仍使用原地址。` });
      onClose();
    } catch (failure) {
  ...
  return (
    <Modal opened title={`导出文件夹：${folder.title}`} onClose={busy ? () => {} : onClose} closeOnClickOutside={!busy} closeOnEscape={!busy} withCloseButton={!busy}>
      <Stack>
        <Text size="sm">将按每篇内容分别捕获服务端已保存版本，并记录各自水位和时间。未确认修改不会包含；这不是文件夹同一时刻的快照。每个一级条目生成独立 ZIP，并附包集清单；导入时需选择整组，解压时将所有分包放在同一目录以保留跨包链接。浏览器可能询问是否允许下载多个文件。</Text>
