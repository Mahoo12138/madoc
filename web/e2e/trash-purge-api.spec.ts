...
test('owner purge requires confirmation and preserves nested batches and asset files', async ({
  page,
}) => {
  ...
  const folder = await (
    await page.request.post(`/api/workspaces/${workspace}/items`, {
      headers,
      data: { type: 'folder', title: 'Parent batch' },
    })
  ).json();
  const nested = await (
    await page.request.post(`/api/workspaces/${workspace}/items`, {
      headers,
      data: { type: 'markdown', title: 'Earlier batch', parentId: folder.id },
    })
  ).json();
  expect(
    (
  ...
  const upload = await page.request.post(
    `/api/workspaces/${workspace}/assets`,
    {
      ...
      multipart: {
        itemId: doc,
        file: { name: 'shared.png', mimeType: 'image/png', buffer: image },
      },
    },
  );
  ...
  expect(
    (
      await page.request.delete(`/api/items/${nested.id}`, { headers })
    ).status(),
  ).toBe(204);
  expect(
    (
  ...
  const selected = batches.find(
    (batch: { root: { id: string } }) => batch.root.id === folder.id,
  );
  const earlier = batches.find(
    (batch: { root: { id: string } }) => batch.root.id === nested.id,
  );
  const details = await (
    await page.request.get(`${list}/${selected.id}/items`)
  ).json();
  expect(details.map((item: { id: string }) => item.id).sort()).toEqual(
    [folder.id, doc].sort(),
  );
  expect(
    (
      await page.request.delete(`${list}/${selected.id}`, {
        data: { confirmation: 'Parent batch' },
      })
    ).status(),
  ).toBe(403);
  expect(
    (
      await page.request.delete(`${list}/${selected.id}`, { headers, data: {} })
    ).status(),
  ).toBe(400);
  expect(
    (
  ...
  expect(
    await (await page.request.get(`/api/assets/${asset.id}`)).body(),
  ).toEqual(image);
  const remaining = await (await page.request.get(list)).json();
  expect(remaining).toHaveLength(1);
  expect(remaining[0].id).toBe(earlier.id);
  expect(
    (await page.request.get(`${list}/${selected.id}/items`)).status(),
  ).toBe(404);
  expect(
    (
      await page.request.post(`${list}/${earlier.id}/restore`, {
        headers,
        data: {},
      })
    ).status(),
  ...
  expect(
    (
      await page.request.post(`${list}/${earlier.id}/restore`, {
        headers,
        data: { destination: { parentId: null } },
      })
    ).status(),
  ).toBe(204);
...
for (const role of ['editor', 'viewer']) {
  test(`${role} cannot purge a batch through the API`, async ({
    page,
    browser,
  }) => {
    ...
    const invite = await (
      await page.request.post(`/api/workspaces/${workspace}/invites`, {
        headers,
        data: { email: `purge-${role}@example.test`, role },
      })
    ).json();
    await page.goto('/workspaces');
    expect(
      (await page.request.delete(`/api/items/${doc}`, { headers })).status(),
    ).toBe(204);
    const list = `/api/workspaces/${workspace}/trash`;
    ...
    try {
      ...
      const response = await context.request.delete(
        `${origin}${list}/${batch.id}`,
        {
          headers: { Origin: origin, 'x-madoc-csrf-token': csrfToken },
          data: { confirmation: batch.root.title },
        },
      );
      expect(response.status()).toBe(403);
      expect(
        (
          await context.request.get(`${origin}${list}/${batch.id}/items`)
        ).status(),
      ...
      if (role === 'viewer') await expect(entry).toHaveCount(0);
        ...
        await entry.click();
        const dialog = memberPage.getByRole('dialog', { name: '回收站' });
        await expect(
      ...
      expect(await (await page.request.get(list)).json()).toHaveLength(
        role === 'viewer' ? 1 : 0,
      );
    } finally {
      await context.close();
