...
test("file tree state survives tab and document changes, with whiteboard fallback", async ({
  ...
  await expect(page.getByRole("navigation", { name: "文档大纲" })).toHaveCount(
    0,
  );
...
test("remote heading edits update a viewer outline and navigation does not write", async ({
  ...
  await viewer.getByRole("dialog").getByRole("tab", { name: "大纲" }).click();
  await viewer
    .getByRole("dialog")
...
test("desktop right outline and mobile drawer navigation", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    outline.getByRole("button", { name: "开始，1 级标题" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "隐藏大纲" }).click();
  await expect(outline).toHaveCount(0);
  await page.getByRole("button", { name: "显示大纲" }).click();
  await expect(outline).toBeVisible();
  ...
  await expect(
    page.getByRole("complementary", { name: "文档大纲面板" }),
  ).toBeHidden();
  await openOutline(page);
  await expect(
  ...
  await expect(
    drawer.getByRole("navigation", { name: "文档大纲" }),
  ).toBeVisible();
