...
前端 Workspace 与编辑器通过引用计数共用单条连接；各自销毁只释放订阅与自身内容房间，
最后一个使用者离开才断开。新订阅者收到当前 online 状态以完成 join，重连重新 watch。
目录通知只刷新 metadata query，不重载正文。打开中内容从目录消失时保留编辑器实例，
切为只读并显示本地副本入口，用户自行离开时沿用本地持久化保护。成员降级同样停止
写入并保留未提交内容，重新打开后使用新的权限。core error 带 itemId，避免其他 Item
的延迟错误影响当前编辑器。
