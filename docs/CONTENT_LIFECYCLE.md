...
物理 DELETE。owner 手动彻底删除和批次条目查看 API 已接入；回收站界面已接入，目录通知与前端订阅已接入。
...
桌面 Workspace 菜单和移动端工具栏提供入口，viewer 不显示入口。owner/editor 可查看批次和内部路径、原位恢复或选择活动文件夹 / 根目录。原目录失效后要求显式选择，恢复后刷新目录。owner 彻底删除必须输入完整名称，失败保留输入供重试；操作期间禁止重复提交和关闭。附件清理仍需引用依据。

目录与编辑器共用连接，收到通知后刷新目录和已打开的回收站；重连会重新读取 metadata。打开中被删除的内容保留为只读救援，不因列表变化直接卸载，用户离开时继续执行本地保存保护。

## 关联导航

搜索、收藏与最近访问均已接入，只展示活动 Item，恢复后重新进入有效导航。
个人访问时间不会因恢复操作本身改变。
...
`trash-ui.spec.ts` 覆盖桌面 / 390px 批次详情、失效父级恢复、完整名称确认、失败重试和空状态；权限用例同时验证 editor 恢复与 viewer 隐藏入口。
