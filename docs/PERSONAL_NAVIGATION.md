...
后台预取、搜索或验证请求错误地改变最近访问。

## 界面

桌面侧栏和手机导航抽屉提供「我的」页，可切换收藏和最近访问。文件树的星标支持
文档、白板和文件夹；viewer 同样可操作自己的收藏。收藏中的文件夹会展开其祖先
并返回文件树，文档和白板直接打开。条目显示当前标题和路径，最近访问显示服务器时间。

Markdown / 白板编辑器准备完成后，每次挂载记录一次访问；初始化失败和只读救援
不会新增成功访问。记录失败显示提示，重新打开后重试。取消收藏不会清除最近访问。
个人查询缓存按工作区和当前账号隔离，目录变化、删除恢复及重连会重新读取。
收藏提交成功后刷新列表，失败保留原状态并允许重试；读取失败显示独立重试入口。
