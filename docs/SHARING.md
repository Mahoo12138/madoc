...
匿名接口只返回固定版本的标题、内容类型、版本名称、发布时刻、正文 / 场景，以及该版本引用附件的最小元数据。不返回 Workspace、Item、创建者或成员信息。公开附件接口在单条 SQL 读取中同时检查 token、有效期、撤销状态、Item 活动状态和附件是否属于当前发布版本。知道同 Workspace 其他资产 ID 不会获得访问权。
