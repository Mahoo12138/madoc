...
- 阶段 4 验收提交 `f22e91a` 完成 `go test ./...`、`go vet ./...`、`go test -race ./...`、前端 typecheck / production build，以及只读分享、评论、成员管理、活动记录浏览器回归。四个独立交付提交为 `e2854fd`、`06ce431`、`33b8a9e`、`f22e91a`。
- 阶段 4 评审修复第一部分：migration 0015 将“历史保留附件”与“版本正文实际引用附件”分开；公开分享的附件元数据与文件接口只授权精确正文清单，正文和清单在同一只读事务读取。评审前已有版本缺少可安全重建的精确清单，因此公开附件默认关闭，需重存手动版本后再发布图片。
- 阶段 4 评审修复第二部分：手动版本只提交从编辑器图片节点解析出的本地 madoc 附件 UUID；同源路径严格解析并拒绝跨站 URL、查询 / 片段、附加路径及非法 UUID。真实上传两张图片后仅引用其一，手动建版和分享读取通过；未引用附件返回 404。5 项分享与解析 Playwright 回归通过。
- 阶段 4 评审修复第三部分：按已确认的“编辑间隔不超过 15 分钟则合并；空闲超过 15 分钟后下一次编辑开启新版本”语义保留滚动窗口。自动版本捕获增加可控时钟入口，并通过连续编辑 10 分钟间隔与空闲 16 分钟的持久化回归，确认连续编辑合并且旧检查点在空闲后不被覆盖。
- 阶段 4 评审修复第四部分：普通复制在同一事务中继承源 Item 附件及已有活动引用到副本 `item_asset_refs`；自动历史快照也采集这些活动引用。回归覆盖源 Item 物理删除、源历史与副本历史清理后，副本仍能读取原附件引用且附件删除受数据库保护。
- 阶段 4 评审修复第五部分：历史版本恢复为副本在数据库事务成功提交后广播 Workspace 目录失效，失败路径不发事件。API 回归验证两条路径，双浏览器历史流程验证另一已连接窗口收到副本并刷新目录。
