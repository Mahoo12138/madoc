...
- 阶段 2 分包导出：文件夹与 Workspace 按根目录一级条目生成独立 ZIP 和 `.package-set.json`，每个 ZIP 有对应 Item 子集的 manifest / 附件；共同根路径保留跨包相对链接，分包文件名确定排序。9 项 E2E 校验整组下载、manifest Item 列表与包集索引逐包一致、跨包 Markdown→白板链接、缺失附件零下载、ZIP 检查器和流式 reader。Go test ./...、go vet ./...、前端 typecheck / production build、diff 空白检查通过；build 保留既有大 chunk 警告。固定单包大小上限和包集导入仍未实现；导入须选择完整包集。
