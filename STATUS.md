...
- 阶段 3 第一部分（已完成，提交待记录）：migration 0010 持久版本模型和手动创建 / 分页 / 读取 API 已实现；覆盖 Markdown 保存投影滞后拒绝、Yjs compaction 后历史不变、白板场景固定、viewer / outsider 读权限与 viewer 写拒绝、附件引用时拒删。`go test ./...`、`go test -race ./...`、`go vet ./...`、前端 typecheck/build 均通过。自动版策略及界面仍未实现。
